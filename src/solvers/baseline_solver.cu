...
int solve_lu_fp64(const double *A_host, const double *b_host, double *x_host,
  ...
  CUDA_CHECK(cudaMemcpy(d_A, A_col_major, n * n * sizeof(double),
                        cudaMemcpyHostToDevice));
  CUDA_CHECK(
      cudaMemcpy(d_b, b_host, n * sizeof(double), cudaMemcpyHostToDevice));
  delete[] A_col_major;
  ...
  int lwork = 0;
  CUSOLVER_CHECK(
      cusolverDnDgetrf_bufferSize(solver_handle, n, n, d_A, n, &lwork));
  ...
  // LU:m A = P*L*U
  CUSOLVER_CHECK(
      cusolverDnDgetrf(solver_handle, n, n, d_A, n, d_work, d_ipiv, d_info));
  ...
  CUSOLVER_CHECK(cusolverDnDgetrs(solver_handle, CUBLAS_OP_N, n, 1, d_A, n,
                                  d_ipiv, d_b, n, d_info));

  // Copy solution back (it's in d_b)
  CUDA_CHECK(
      cudaMemcpy(x_host, d_b, n * sizeof(double), cudaMemcpyDeviceToHost));
...
double compute_residual(const double *A_host, const double *x_host,
                        const double *b_host, int n, double *r_host) {
